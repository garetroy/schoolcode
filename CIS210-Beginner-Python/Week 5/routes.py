...
Consulted with on design only: Tu Rui
"""
...
def dfs(place, dist_so_far, roads, distances):
    ...
    if place not in distances or distances[place] > dist_so_far: #checks to see if place is already been to
      distances[place] = dist_so_far #sets the place distance to the distance so far
      for paths in roads[place]:
        #it adds the paths from the city to other cities and their distances
        city, dist = paths 
