...
Consulted with: Liz Olson, Andrew Hill, Rui Tu
...
class Appt:
    ...
    def __lt__(self, other):
        ...
        if self.end <= other.begin:
            return True
        return False
        # Note that __gt__ is defined in terms of __lt__, so __lt__
        # must NOT be defined in terms of __gt__ (to prevent infinite loop)
    ...
    def overlaps(self, other):
		...
		Arguments:
            other: another Appt
        Returns:
            False if there is no non-zero overlap between appointments       
        """
    ...
    def intersect(self, other, desc=""):
        ...
        return Appt(self.begin.date(), max(self.begin.time(), other.begin.time()), min(self.end.time(), other.end.time()), desc) 
...
class Agenda:
    ...
    def intersect(self,other,desc=""): 
        ...
        result = Agenda()
        for self_appointments in self.appts:
            for other_appointments in other.appts:
                if self_appointments.overlaps(other_appointments):
                    result.append(self_appointments.intersect(other_appointments))
    ...
    def normalize(self):
        begin = self.appts[0].begin.time()
        end = self.appts[0].end.time()
        text = self.appts[0].desc
        result = Agenda()
        for appt in self.appts[1:]:
            b2, e2, t2 = appt.begin.time(), appt.end.time(), appt.desc
            if b2 > end: 
                result.append(Appt(self.appts[0].begin.date(), begin, end, text) )
                begin, end, text = b2, e2, t2
            else: 
                end = max(end, e2)
                text = text + "; " + t2
        result.append(Appt(self.appts[0].begin.date(), begin, end, text))
    ...
    def complement(self, freeblock):
        """
        Returns an Agenda object that is the free time.

        Args:
            freeblock
        """
        new_lst = self.normalize()
        final = Agenda()
        begin = freeblock.begin.time()
        for appt in new_lst:
            if begin < appt.begin.time():
                final.append(Appt(appt.begin.date(), begin, appt.begin.time(), appt.desc))
            begin = appt.end.time()
        if begin < freeblock.end.time():
            final.append(Appt(freeblock.begin.date(), begin, freeblock.end.time(), freeblock.desc))
        return final
