...
struct pixel {
	...
	pixel(double r, double g, double b) : red(r), green(g), blue(b) {};
        pixel operator*(pixel& in) const
        {
            return pixel(red*in.red,green*in.green,blue*in.blue);
        }
        pixel operator+(pixel& in) const
        {
            return pixel(red+in.red,green+in.green,blue+in.blue);
...
void apply_stencil(const int radius, const double stddev,
  ...
  double xkernal[9];
  double ykernal[9];
  prewittX_kernel(3,3,xkernal);
  prewittY_kernel(3,3,ykernal);
  ...
  for(int i = 0; i < rows; ++i) {
	...
	for(int y = j - 1, ky = 0; y <= j + 1; ++y, ++ky) {
	  if(x >= 0 && x < rows && y >= 0 && y < cols) {
	    const int in_offset = x + (y*rows);
	    const int k_offset = kx + (ky*dim);
            //applys greyscale also
	    xintense += xkernal[k_offset] * (in[in_offset].red + in[in_offset].green +
                                             in[in_offset].blue)/3;
	    yintense += ykernal[k_offset] * (in[in_offset].red + in[in_offset].green +
                                             in[in_offset].blue)/3;
	  }
	}
      }
      
      double total = sqrt(xintense * xintense + yintense*yintense);
      out[out_offset].red   = total;
      out[out_offset].blue  = total;
      out[out_offset].green = total;
    }    
  }
}
