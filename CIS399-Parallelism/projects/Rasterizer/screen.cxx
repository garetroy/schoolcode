...
Screen::Screen(unsigned char* _buffer, int _width, int _height, double positionx, double positiony)
{
    ...
    //Speedup of .01s
    #pragma omp parallell for num_threads(3)
...
Screen::SetRow(double x1, double x2, const double y)
{
    ...
    int x1i = (int) x1;
    int x2i = (int) x2;
    #pragma omp parallel for num_threads(3) firstprivate(proportion)
    for(int i = x1i;i <= x2i; i++){
        proportion = ((double)i - startx1) / (startx2 - startx1);
        if(SetZCoord(i,y,proportion) == 1){
...
Screen::SetTriangle(Triangle *t)
{
    ...
    double x1 = 0, x2 = 0, proportion = 0;
    int minycoord = 0, maxycoord = 0;

    if(t->triangletype.flatbottom){
            minycoord = ceil441(t->coord[0].y);
            maxycoord = floor441(t->coord[1].y);
            #pragma omp parallel for num_threads(3) firstprivate(x1,x2,proportion) 
            for(int minycoordi = (int) minycoord; minycoordi <= maxycoord; minycoordi++){
                proportion = (minycoordi - t->coord[0].y) / (t->coord[1].y - t->coord[0].y);
    ...
    if(t->triangletype.flattop){
            ...
            #pragma omp parallel for num_threads(3) firstprivate(x1,x2,proportion) 
            for(int maxycoordi = (int) maxycoord; maxycoordi >= minycoord; maxycoordi--){
                proportion = (t->coord[0].y - maxycoordi) / (t->coord[0].y - t->coord[1].y);
                x1 = Lerp(t->coord[0].x, t->coord[1].x, proportion);
                x2 = Lerp(t->coord[2].x, t->coord[1].x, proportion);
                SetBoundColors(t,proportion);
                SetBoundShading(t,proportion);
                SetBoundZ(t,proportion);
                SetRow(x1,x2,maxycoordi);
            }
    }
}
