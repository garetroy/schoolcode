...
Filter::Update(){
	...
	char msg[256];
	SetStart();
	sprintf(msg, "%s: about to update input1, (starting timer)", sourcename); Logger::LogEvent(msg);
	if(imagein1 != NULL) imagein1->Update();
	sprintf(msg, "%s: done updating input1, took: %fs", sourcename,GetTime()); Logger::LogEvent(msg);
	SetStart();
	sprintf(msg, "%s: about to update input2, (starting timer)", sourcename); Logger::LogEvent(msg);
	if(imagein2 != NULL) imagein2->Update();{
	sprintf(msg, "%s: done updating input2, took: %fs", sourcename,GetTime()); Logger::LogEvent(msg);
	}
	Execute();
}
